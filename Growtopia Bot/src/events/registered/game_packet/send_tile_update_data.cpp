...
namespace events {
	void send_tile_update_data(EventContext& ctx) {
        ...
        int index = ctx.m_game_packet->m_tile_pos_x + ctx.m_game_packet->m_tile_pos_y * ctx.m_client->m_world.m_width;
        if (index >= 0 && index < ctx.m_client->m_world.m_tiles.size()) {
            auto& tile = ctx.m_client->m_world.m_tiles[index];
