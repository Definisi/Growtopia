...
std::string generate_klv(float game_version, std::uint32_t protocol, std::int32_t hash, const std::string& rid)
{
    std::cout << "Generate KLV :\n" << game_version << "\n" << protocol << "\n" << hash << "\n" << rid << std::endl;
    std::string game_version_formatted = std::format("{:.2f}", game_version);
    std::array salts = {
