...
public:
	...
	NOTIFY_INLINE auto get_default_title() -> char*
	{
		if (!strlen(this->title))
		{
			...
			case ImGuiToastType_Warning:
				return const_cast<char*>("Warning");
			case ImGuiToastType_Error:
				return const_cast<char*>("Error");
			case ImGuiToastType_Info:
				return const_cast<char*>("Info");
			}
		}
	...
	NOTIFY_INLINE auto get_color() -> const ImVec4&
	{
		...
		case 1:
			return { 0.36f, 0.72f, 0.36f, 1.0f }; // Green
		case 2:
			return { 0.85f, 0.33f, 0.31f, 1.0f }; // Error
		case 3:
			return { 0.94f, 0.68f, 0.31f, 1.0f }; // Yellow
		case 4:
			return { 0.26f, 0.55f, 0.79f, 1.0f }; // Blue
		default:
			return { 1.0f, 1.0f, 1.0f, 1.0f }; // White
		}
	}
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			if (GetWindowPos().y + GetWindowHeight() - NOTIFY_PADDING_Y - height < GetWindowPos().y + 110) {
			...
			SetNextWindowPos(ImVec2(GetWindowPos().x + GetWindowWidth() - NOTIFY_PADDING_X, GetWindowPos().y + GetWindowHeight() - NOTIFY_PADDING_Y - height), ImGuiCond_None, ImVec2(1.0f, 1.0f));
			//SetNextWindowPos(ImVec2(0, 0));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(GetWindowPos().x + GetWindowWidth() / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
