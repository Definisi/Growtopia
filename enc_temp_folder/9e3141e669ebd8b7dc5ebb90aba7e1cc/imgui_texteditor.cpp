...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		std::string output = (pos == std::string::npos) ? lln : lln.substr(pos + 1);

		std::cout << output << std::endl;
		std::vector<std::string> matches = AutoComplete(output);
		...
		if (!matches.empty()) {
			...
			ImGui::BeginTooltip();
			for (const auto& match : matches) {
				if (ImGui::Selectable(match.c_str())) {
					std::cout << "KEPENCET: " << match << std::endl;
...
std::vector<std::string> TextEditor::AutoComplete(const std::string& input) {
	std::vector<std::string> matches;
	if (input.empty()) {
		return matches;
	}
	for (const auto& identifier : mLanguageDefinition.mIdentifiers) {
		if (std::string(identifier.first).find(input) == 0) {
			matches.push_back(identifier.first);
		}
	}
	return matches;
}
